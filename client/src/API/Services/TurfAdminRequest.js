...
export const RuleDeleteReqApi = (data, token) => {
  ...
  try {
    const response = AxiosTurfAdmin.delete(
      `/rule-delete?index=${data.deleteId}&id=${data.id}`,
      {
        headers: { Authorization: "Bearer " + token },
      }
    );
...
export const RuleUpdateFindReqApi = (data, token) => {
  console.log(data, "data", token, "token");
  try {
    const response = AxiosTurfAdmin.get(
      `/rule-update-find?id=${data.id}&index=${data.index}`,
      {
        headers: { Authorization: "Bearer " + token },
      }
    );
...
export const GroundDetailSubmitReqApi = async (data, id, token) => {
  ...
  try {
    const response = AxiosTurfAdmin.post(
      `/ground-detail-form-submit?id=${id}`,
      data,
      {
        headers: { Authorization: "Bearer " + token },
      }
